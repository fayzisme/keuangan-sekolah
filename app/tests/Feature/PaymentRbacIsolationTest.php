...
it('payments index scoped to school (no cross-school leak)', function () {
    ...
    $paymentB = \App\Domain\Billing\Models\Payment::create([
...
it('creator cannot verify own payment -> 409 maker-checker', function () {
    ...
    $payment = \App\Domain\Billing\Models\Payment::firstOrFail();
...
function seededSchool(string $name = 'SMA A')
{
    ...
    $bt = \App\Domain\Billing\Models\BillType::create(['school_id' => $school->id, 'name' => 'SPP', 'tipe_bayar' => 'monthly', 'tarif_cents' => 300000]);
    $student = \App\Domain\Student\Models\Student::create(['school_id' => $school->id, 'nis' => 'A-1', 'name' => 'Siswa A', 'is_active' => true]);
    $invoice = \App\Domain\Billing\Models\BillingInvoice::create([
