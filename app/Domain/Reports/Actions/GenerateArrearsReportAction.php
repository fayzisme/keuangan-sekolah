...
final class GenerateArrearsReportAction
{
    ...
    {
        // Base query: invoices sekolah ini, hanya yang masih OPEN/PARTIAL
        ...
        foreach ($invoices as $inv) {
            ...
            $results[] = [
                ...
                'kelas' => $inv->student->class_id ? $inv->student->classRoom->name ?? '' : '',
        ...
        return [
            'filters' => $filters,
            'total_siswa_tunggak' => count(array_unique(array_column($results, 'student_id'))),
            'total_tunggakan_cents' => array_sum(array_column($results, 'sisa_cents')),
            'data' => $results,
        ];
    }
}
