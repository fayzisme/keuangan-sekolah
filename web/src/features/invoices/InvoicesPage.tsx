...
export function InvoicesPage() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(`✓ ${res.message ?? `Buat ${res.count ?? 0} tagihan.`}`);
      setGenBillType('');
      setGenBulan('');
      setGenDue('');
      load();
    } catch (err) {
      setGenError(err instanceof Error ? err.message : 'Buat tagihan gagal.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleVoid = async (id: number) => {
    if (!confirm('Batal tagihan ini? Aksi tidak bisa undo.')) return;
    try {
      await invoiceVoidApi(token ?? '', id);
      load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Batal tagihan gagal.');
    }
  };
  ...
  return (
    <>
      <div className="page-head">
        <h2>Tagihan</h2>
        <p>Buat tagihan bulanan (monthly) atau sekali-payi (one-time), dan lihat status tagihan.</p>
      </div>
      ...
      <div className="grid-2" style={{ marginTop: '1.25rem' }}>
        <Card
          title="Buat Tagihan"
          sub="Tagihan bulanan untuk tipe monthly, tagihan sekali untuk one-time"
          actions={<span className="badge badge-purple">{billTypes.length} tipe</span>}
        >
          ...
          <form onSubmit={handleGenerate} style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem', marginTop: '0.5rem' }}>
            <div className="form-grid">
              ...
              <div className="field">
                <label htmlFor="gen-due">Batas Pembayaran</label>
        ...
        <Card title="Daftar Tagihan" sub="Filter status &amp; periode tahun">
          ...
          <div className="table-scroll">
            <table className="table">
              ...
              <tbody>
                {invoices.length === 0 && EmptyRow(8, 'Belum ada tagihan untuk filter ini.')}
                {invoices.map((inv) => (
                  <tr key={inv.id}>
                    <td>{inv.id}</td>
                    ...
                    <td>
                      {inv.status === 'OPEN' && (
                        <button className="btn btn-danger" onClick={() => handleVoid(inv.id)}>Batal</button>
                      )}
                    </td>
                  </tr>
                ))}
